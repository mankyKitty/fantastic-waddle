...
** Manage change
   - Maintain our ~GOL~ record in a ~Dynamic~
   - At it will change over time
   - We let Reflex manage that for us
...
** ~Event~ to draw
   ...
   /NOTE:/ Use of ~dGOL~ in this definition.
** Render a Step  
   #+BEGIN_SRC haskell
     step :: ... => WebGLRenderingContext -> GOL -> m GOL
   #+END_SRC
   #+INCLUDE: "../frontend/src/WebGL/GOL.hs" :LINES "210-211" src haskell
